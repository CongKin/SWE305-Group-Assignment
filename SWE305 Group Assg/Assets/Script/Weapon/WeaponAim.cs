...
public class WeaponAim : MonoBehaviour
{
    ...
    public float CurentAimAngle { get; set; }
    ...
    public void RotateWeapon()
    {
        ...
        {
            // Get Angle
            CurentAimAngle = Mathf.Atan2(currentAim.y, currentAim.x) * Mathf.Rad2Deg;
            CurrentAimAngleAbsolute = Mathf.Atan2(currentAimAbsolute.y, currentAimAbsolute.x) * Mathf.Rad2Deg;

            // Clamp our rotation
            if(weapon.WeaponOwner.GetComponent<CharacterFlip>().FacingRight)
            {
                CurentAimAngle = Mathf.Clamp(CurentAimAngle, -180, 180);
            }
            else
            {
                CurentAimAngle = Mathf.Clamp(CurentAimAngle, -180, 180);
            ...
            // Apply the angle
            lookRotation = Quaternion.Euler(CurentAimAngle * Vector3.forward);
            transform.rotation = lookRotation;
        }
        else
        {
            CurentAimAngle = 0f; // If the mouse is not moving at all at the begining
